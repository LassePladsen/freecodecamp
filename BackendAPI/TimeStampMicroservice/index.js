...
app.get("/api/:date?", function (req, res) {
  ...
  if (typeof req.params.date === "undefined") {
    ...
    if (Number.isNaN(dateNumber)) {  // is not a number after cast: should be a GMT time format
      date = new Date(req.params.date);
    } else {  // ms format use Number
      date = new Date(dateNumber);
    }
  }

  // If not found
  if (date.valueOf() == "Invalid Date" || typeof date === "undefined") {
